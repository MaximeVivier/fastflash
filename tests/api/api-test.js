...
const all_fields_return_from_graphql =
  'id\nrecto\nverso\nrecto_def\nverso_def\nposition\nlibrary';
...
describe('Test all graphql routes', () => {
  ...
  test('graphql all_cards', async (done) => {
    ...
    expect(all_cards.sort((a, b) => a.id - b.id)).toEqual(
      data_for_test.sort((a, b) => a.id - b.id)
    );
  ...
  test('graphql card', async (done) => {
    ...
    const data_for_fetch_card_test = data_for_test.find(
      (el) => el.id === id_of_the_card_to_fetch
    );
  ...
  test('graphql flipCard', async (done) => {
    ...
    const data_for_flip_card_test = data_for_test.find(
      (el) => el.id === id_of_the_card_to_flip
    );

    const str_card_to_flip_query = `mutation{
      flipCard(
        id:${data_for_flip_card_test.id},
        position:${data_for_flip_card_test.position}
      ) {id} }`;

    await request.post('/graphql').send({ query: str_card_to_flip_query });
